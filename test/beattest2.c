...
static guint64 calculateEnergy(const unsigned int *bufftmp) {
        guint64 e = 0;
        int c = 0;

        for(c = 0; c < 1024; c += 2) {
                /* e = sum(left[k]^2 + right[k]^2, k=0...1024) */
                e += ( bufftmp[c] * bufftmp[c] ) + ( bufftmp[c+1] * bufftmp[c+1] );
        }
...
static void playDSP(audio_info *ai) {
	...
	int t = 0;
        ...
        for( t = 0; t < count; t++) {
                ...
                e = calculateEnergy((const unsigned int *)(ai->buffer + (2 * 1024 * count * sizeof(char))));
                printf("loop = %d\tt = %d\tEnergy = %llu\n", loop, t, e);
