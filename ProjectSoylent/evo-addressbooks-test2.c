 ...
 * Or use filters using
 *      ./evo-addressbooks-test "(contains \"full_name\" \"John Doe\")"
 * or some other valid EBook query
 */
 ...
 */
static void processBook(EBook *book, const gchar *q) {
        gboolean b;
        GError *err = NULL;
        ...
        if(q == NULL) {
                ...
                query = e_book_query_from_string (q);
        }

        if(query == NULL) {
                g_warning("Error generating EBookQuery\n");
                return;
        }
...
int main(int argc, char *argv[]) {
        ...
        /* Give some help message */
        if(argc < 2) {
                g_print("\tYou can give a query using\n");
                g_print("\t\t%s \"(contains \\\"full_name\\\" \\\"John Doe\\\")\"\n", argv[0]);
                g_print("\tOr using another valid ebook (escaped) filter string\n\n");
        }
        ...
        for(l = list; l!= NULL; l = l->next) {
                ...
                for(s = groups; s != NULL; s = s->next) {
                        ...
                        if(argc < 2) {
                                processBook(book, NULL);
                        }
                        else {
                                g_print("Querying using \"%s\"\n", argv[1]);
                                processBook(book, argv[1]);
                        }
